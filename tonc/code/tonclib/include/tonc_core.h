...
INLINE uint numFourByteChunks(uint size);
...
INLINE void dma_cpy(void *dst, const void *src, uint count, uint ch, u32 mode)
{
	REG_DMA[ch].cnt = 0; // first step is to shut off any previous transfers
	REG_DMA[ch].src = src;
	REG_DMA[ch].dst = dst;
	REG_DMA[ch].cnt = mode | count; 	// count fills the first N bits, mode fills the top 16
}
...
INLINE void dma3_cpy(void *dst, const void *src, uint size)
{
	dma_cpy(dst, src, numFourByteChunks(size), 3, DMA_CPY32);
}

static inline uint numFourByteChunks(uint size) {
	// 32 bits = 4 bytes
	// numBytes / 4 = num 32b chunks in 
	return size / 4;
}
